...
//   Updated: 2016/01/17 14:46:20 by ngoguey          ###   ########.fr       //
//                                                                            //
// ************************************************************************** //
...
static auto build_files_and_tests(
	ComboGenerator &cg, char const *const av[], int const ntests)
	-> std::vector<UnitTest>
{
	std::vector<UnitTest> tasks;
	std::vector<Piece const *> pcvec(ntests);
	auto it = cg.begin();
...
std::vector<UnitTest> build_tasks(char const *const av[])
{
	...
	return build_files_and_tests(cg, av, ntests);
}
